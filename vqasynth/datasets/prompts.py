...
from vqasynth.datasets.pointcloud import human_like_distance, calculate_distances_between_point_clouds, calculate_centroid
from vqasynth.datasets.mcq_templates import *
...
def volume_data(A, B=None):
    ...
    extent_A = A[1].get_axis_aligned_bounding_box().get_extent()
    volume_A = extent_A[0] * extent_A[1] * extent_A[2]
...
def angle_data(A, B):
    ...
    angle_rad = math.atan2(delta_x, delta_z) 
...
def angle2agent(A, B=None):
    ...
    angle_rad = math.atan2(A_center[0], A_center[2]) 
    angle_degrees = math.degrees(angle_rad)
    angle_degrees = round(angle_degrees, 2)
...
def distance2agent(A, B=None):
    ...
    A_center = A[1].get_center()
    distance = np.linalg.norm(A_center)
...
def evaluate_predicates_on_pairs(pairs, is_canonicalized):
    ...
    all_prompt_variants = qualitative_prompts + quantitative_prompts + embodied_prompts + multi_qa_prompts + tf_qa_prompts
    selected_prompt_choices = random.sample(all_prompt_variants, 10)
    results = []
    qa_information = []
